...
def populate_dim_municipality(engine):
    ...
    demo_df = pd.read_csv(os.path.join(SILVER_PATH, 'demographics.csv'), encoding='utf-8')
    ...
    stores_df = pd.read_csv(os.path.join(SILVER_PATH, 'stores.csv'), encoding='utf-8')[["municipality_name", "municipality_code"]].rename(columns={"municipality_name": "name"})
    
    # 3. Read from tourism.csv
    tourism_df = pd.read_csv(os.path.join(SILVER_PATH, 'tourism.csv'),encoding='utf-8')[["municipality_name", "municipality_code"]].rename(columns={"municipality_name": "name"})
...
def populate_dim_product(engine):
    ...
    df = pd.read_csv(os.path.join(SILVER_PATH, 'products.csv'), encoding='utf-8')
    ...
    dim_df = df[['product_id', 'name', 'category', 'unit_price', 'unit_type', 'supplier']]
...
def populate_fact_sales(engine, date_map, product_map, store_map):
    ...
    for i, chunk in enumerate(pd.read_csv(sales_file, chunksize=chunk_size, encoding='utf-8')):
        ...
        chunk['date_key'] = pd.to_datetime(chunk['date']).dt.strftime('%Y%m%d').astype(int).map(date_map)
        ...
        fact_df = chunk[['date_key', 'product_key', 'store_key', 'sales_amount', 'units_sold']]
...
def main():
    ...
    municipality_map = get_dimension_map(engine, 'dim_municipality', 'name', 'municipality_key')
    date_map = get_dimension_map(engine, 'dim_date', 'date_key', 'date_key') # Simple 1:1 map
    product_map = get_dimension_map(engine, 'dim_product', 'product_id', 'product_key')
    ...
    # Note: Tourism and Demographics facts are left as an exercise.
    # The logic would be very similar to populate_fact_sales.
    populate_fact_sales(engine, date_map, product_map, store_map)
    
    print("\nETL process for 'fact_sales' completed successfully!")
